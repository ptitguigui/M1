...
open_ifile(file_desc_t *fd, unsigned int inumber)
{
	...
	read_bloc(current_volume, first_bloc, BLOC_SIZE, fd->fds_buf);
...
flush_ifile(file_desc_t *fd)
{
	...
	write_bloc(current_volume, vbloc, BLOC_SIZE, fd->fds_buf);
...
{     
	...
	if (! vbloc)
	    ...
	    /* load the bloc */
	    read_bloc(current_volume, vbloc, BLOC_SIZE, fd->fds_buf);
    }
}
...
writec_ifile(file_desc_t *fd, char c)
{
	...
	write_bloc(current_volume, ibloc, BLOC_SIZE, fd->fds_buf);
	/* read the new buffer */
	...
	if (! ibloc) 
	    ...
	    read_bloc(current_volume, ibloc, BLOC_SIZE, fd->fds_buf);
	fd->fds_dirty = FALSE;
    }

						...
						if (wtg.who.contains("delete")) {
							...
							if (wtg.Data.get(fils.get(i)).contains(noeud)) {
								if (wtg.Data.get(noeud).size() > 1) {
									...
									System.out.println("Le noeud_" + noeud + " est remplacé par le noeud_"
											+ wtg.Data.get(noeud).get(0) + " dans la liste des noeuds relié au noeud_"
											+ fils.get(i));
							...
							if (!wtg.Visited.containsValue(false)) {
								...
								System.out.println(
										"Le noeud_ " + noeud + " n'est plus accessible dans les datas et Visited");
							}
